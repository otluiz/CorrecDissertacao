...
A origem dos dados, os ``inputs'' estão representados na figura onde se lê ``Dados'' este está repleto de \textit{missing data} e/ou dados inconsistentes, conhecidos como dados não estruturados. 
...
Na pesquisa apresentada nessa dissertação, particularmente, as arvores de decisao possibilitaram grandes avancos na proposicao do modelo de prediccao, conforme pode ser observado no capitulo dedicado aos resultados. 
...
A origem das árvores de decisão, como algoritmo no campo da inteligência artificial não é muito explorada na comunidade científica. Boa parte das referencias encontradas (FONTE – dissertação da menina da Federal) apontam que as árvores de decisão foram propostas por Ross Quinlan, pesquisador australiano, no final da década de 70 e início dos anos 80, sendo o ano de 1983 aquele em que foi apresentado o primeiro algoritmo para geração de árvores de decisão: o ID3 (Iterative Dichotomiser),  utilizado até hoje e considerado um dos mais importantes.  Todavia, HSSINA; MERBOUHA; MOHAMMED, 2014, sugerem que autores no campo da estatística descrevem que seu surgimento é da década de 60, com Sonquist e Morgan, que utilizaram árvores de decisão para predição e explicaçãoo, com o algoritmo AID (Automatic Interation Detection). A partir desse modelo, houve uma expansão para problemas de classificação e discriminação, cuja abordagem teria culminado no CART (Classification and Regression Tree ), método desenvolvido por Breiman e seus colaboradores.
Quinlan (1986) discute que desde que a inteligência artificial começou a se desenvolver como campo de teorização e investigação, em meados dos anos 50, as máquinas de aprendizagem (machine learning) ocuparam um lugar de particular interesse dos pesquisadores, sobretudo pelo interesse na compreensão e modelização de comportamentos inteligentes. Tal interesse, ainda segundo esse autor, instigou a busca pelo desenvolvimento de sistemas competentes baseados no conhecimento (knowledge-based) e tomou vulto nas pesquisas em inteligência artificial. Quinlan avança, pontuando o interesse de muitos pesquisadores, dos quais ele próprio, no que ele chamou, nesse seu importante artigo, de “microcosmo de máquinas de aprendizagem e de uma família de sistemas de aprendizagem que têm sido utilizadas para construer sistemas baseado em conhecimentos de um tipo simples” (livre tradução do autor). 
O primeiro grupo de árvores de decisão, aind conforme Quinlan (1986) eram responsáveis por tarefas de classificação, desenvolvendo uma decisão do topo da árvore para baixo - das raízes as folhas - ou seja: top down. (Note-se que numa árvore de decisão, embora tenhamos, igualmente, seu desenvolvimento das raízes às folhas, a sua representação começa pelo nó-raiz, na parte superior, descendo em direção às folhas ISSO TAMBÉM PODERIA SER UMA NOTA DE RODAPÉ). O modelo proposto comportaria – como o é até hoje – inúmeras análises e reanálises, em todos os estágios e durante todo o processo. Assim, esse primeiro modelo faria parte da família de algoritmos do tipo Top-Down Induction of Decision – TDID, representada por Quinlan, em forma de árvore, na figura a seguir (p. 84).

FIGURA

Conforme mencionado, a tarefa das árvores de decisão desse tipo é a de classificação.  Seu produto sera, pois, uma classe. Para tal, Quinlan (1986) descreve que a estratégia subjacente a essa árvore é não-incremental. Ou seja, um grupo de casos relevantes é apresentado, os exemplos são dados, mas não há uma ordem específica de apresentação dos exemplos. Possivelmente, em virtude dessa estratégia, muitos autores relacionam a tarefa de classificação de uma árvore de decisão à aprendizagem não supervisionada. De maneira complementar a essa ideia, quando a árvore de decisão assume a função de predição, fala-se de aprendizagem supervisionada.
...
Uma árvore de decisão obedece á regra básica ''se-então'', partindo do nós (SE) para as folhas (ENTÃO). O conhecimento é representado em cada nó, que apresenta pelo menos duas saídas ou ramificações possíveis, podendo ou não converter-se em um novo nó, relacionado a um novo nível.
Embora seja um algoritmo simples e de fácil interpretação, uma das mais importantes questões a ser consideradas diz respeito a como propor as regras de forma adequada e relevante para a geração da árvore. É função do algoritmo identificar o melhor atributo, que será responsável por criar o nó de decisão. As ligações entre os nós representam valores possíveis do teste do nó superior e as folhas indicam a classe (categoria) a qual o registro pertence (CAMILO, SILVA, 2009).
...
Em outro artigo que também se interessou por conteúdos de natureza mais acadêmica, 
conduzido por (FONTE - ANO), 
...
os autores justificaram que as mídias sociais têm sido utilizadas cada vez mais para promoção das bibliotecas, com o objetivo de incrementar a relação com os clientes, permitindo “compartilhar informações e conhecimentos, incrementar serviços e promoções, interação com estudantes usuários das bibliotecas, a um custo mínimo” (Chue \& Due, 2013, p. 72).
 Tal prática têm promovido considerável mudança na interação com usuários e relacionamento com os clientes (Del Bosque, Leif e Skaf, 2012; Cavanagh, 2015), tendo sido utilizado frequentemente como alternativa para estabelecer uma conexão personalizada com os seus usuários (Boaten \& Quan, 2014). 
